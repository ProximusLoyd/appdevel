#!/usr/bin/env python
import os , sys , stat , unicodedata
...
,}
FILE_TYPE_MAP = {
    "music": [".mp3", ".wav", ".flac", ".ogg"], "video": [".mp4", ".mkv", ".avi", ".mov"], "image": [".jpg", ".jpeg", ".png", ".gif", ".bmp"], "archive": [".zip", ".tar", ".gz", ".rar", ".7z"], "text": [".txt", ".md", ".doc", ".docx", ".pdf"], "code": [".py", ".js", ".html", ".css", ".rs", ".c", ".cpp", ".java", ".sh"], "config": [".toml", ".json", ".yaml", ".yml", ".ini", ".cfg", ".lock"]
,}
        ...
        if stat.S_ISDIR(mode):
            ...
            for type_name, extensions in FILE_TYPE_MAP.items():
                if ext in extensions:
                    ...
                    break
        return {
            "name": f"{color}{glyph}{COLORS['reset']}  {entry.name}", "format": format, "type": file_type.capitalize(), "perms": perms, "size": format_size(size), "modified": modified, "is_dir": stat.S_ISDIR(mode),}
    except OSError:
        return None
